...
//       const totalUsers = monthUserTotals[month] || 1
//       const totalPV = monthPVTotals[month] || 1
...
export function transformHeatmap(
  ...
  rows.forEach(row => {
    ...
    /* FIX: avoid duplicate accumulation */
    if (!monthUserTotals[month]) {
      monthUserTotals[month] = users
    }

    if (!monthPVTotals[month]) {
      monthPVTotals[month] = pv
    }
  })
  ...
  rows.forEach(row => {
    ...
    /* FIX: prevent duplicate summation */
    if (!modelUsers[model][month]) {
      modelUsers[model][month] = users
    }

    if (!modelPV[model][month]) {
      modelPV[model][month] = pv
    }
  })
